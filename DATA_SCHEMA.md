...
| `roadmap_opportunity_owners` | BaseWithoutTenant | `id` (uuid), `name` (text UNIQUE), `position` (int, default 0) | Legacy taxonomy. New human assignments use `ownerUserId` and must be an Ally super-admin. One exception, seeded by migration `1913000000000`: `'Bug Hunter Agent'`, written to the legacy `owner` column (never `ownerUserId`) by `BugFixSessionService.releaseLinkedRoadmapOpportunity` — see `bug_findings` below |
