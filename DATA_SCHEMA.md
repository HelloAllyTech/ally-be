...
| `language_glossary_sections` | BaseWithoutTenant | `language_id` → `languages`, `section_code`, `profileId` (uuid nullable → `language_variety_profiles`; NULL = global row, non-NULL = variety-profile overlay served merged with global, overlay wins on section_code), `content` (markdown), `injection_mode` (`always`/`retrieved`; reassigned each cycle by the computed tier pass — value-per-token knapsack under the Tier 0 cap), `tierPinned` (bool: admin override, set automatically on a manual mode change; the tier pass never touches pinned rows), `status`, `version`, `entries` (jsonb proposals; each entry's `provenance` carries `annotationIds`, `tenantIds`, `batchId` and lexical `evidence` {say/avoid corpus counts, verdict}, and its `adjudication` carries the unattended-verdict state: `rejectVotes` (consecutive reject votes — a reject is permanent, so it needs two) plus `deferrals`/`lastDeferredAt`/`lastDeferReason` (consecutive deferrals for the same digit-insensitive reason, driving an exponential re-adjudication backoff capped at a week — a deferral leaves the entry `proposed`, so without it an unchangeable verdict is re-billed hourly)), `importance` (⚠️ write-only: consolidation sets it, nothing reads it — placement comes from the tier knapsack) | Per-language "how to speak X" glossary served to the live agent (Tier 0 style card / Tier 1 retrieval). Consolidation reads exclude `isTestOrganization` tenants; extraction is construct-class clustered with support + distributional-evidence gates |
...
| Table | Base | Key columns | Notes |
|-------|------|-------------|-------|
| `scenarios` | BaseWithoutTenant | `id` (int), `title`, `scenario`, `description`, `cover_image_url`, `cover_video_url`, `status` (`ScenarioStatus`, default DRAFT), `prompt`, `difficulty_level`, `is_global`, `is_public`, `competency_id`, `competencyIds` (jsonb `uuid[]`), `metadata`/`translations` (jsonb), `engine` (`ScenarioEngine`: `SIMULATION` — single-valued since §3.9 was removed), `roleplaySpecId` (uuid, vestigial, always null), `category` (`ScenarioCategory`: `ORIGINALS`\|`DEMO`\|`PARTNER_SIM`\|`OTHER`, nullable), `partnerOrgName` (varchar 255, nullable free-text tag), `deleted_at` | The core training scenario. `category`/`partnerOrgName` organise the Studio list (filterable; admin search also matches the partner tag). `competencyIds` is the full set of competencies the simulation assesses — selecting a cluster in the builder stores it **expanded**, never as the cluster — and `competency_id` always mirrors `competencyIds[0]` for readers that predate the array (see `resolveCompetencySelection`) |
...
| `competencies` | BaseWithoutTenant | `id` (uuid), `name`, `isCustom` (bool, default false), `createdBy` (int, nullable) | Higher-order skill groupings. `isCustom` rows are user-private: created on the fly when a simulation's behaviour selections diverge from a defined competency's mapping, named `{userId}_custom_{N}`, hidden from the global list and never placed in a cluster |
| `competency_behaviors` | BaseWithoutTenant | `id` (uuid), `competencyId` (idx), `behaviorId`, `type` (`HELPFUL`\|`UNHELPFUL`) | Which behaviours count for/against a competency. Uniq `(competencyId, behaviorId)` — a behaviour maps at most once per competency, so one listed as both is kept as HELPFUL. Selecting the competency in the builder fills the Behaviour Instructions / Scoring Rubric table from these; an empty mapping is seeded on first read from `COMPETENCY_BEHAVIOR_INSTRUCTION_PRESETS` |
| `competency_clusters` | BaseWithoutTenant | `id` (uuid), `name` (uniq on `LOWER(name)`), `createdBy` (int, nullable) | A named grouping of competencies — typically a framework the customer already trains against. **Ships empty**: clusters are created by admins in the Competencies tab, never seeded, because which competencies belong together is their call. An **authoring** grouping only: picking a cluster in the builder expands to its members and the scenario stores those ids, so re-clustering can never change what a published simulation assesses (and deleting a cluster is safe) |
| `competency_cluster_members` | BaseWithoutTenant | `id` (uuid), `clusterId` (idx), `competencyId` (idx) | Cluster membership, uniq `(clusterId, competencyId)`. Deliberately **many-to-many**: an admin curating frameworks decides how they overlap, so one competency may sit in several clusters. Both FKs cascade on delete |
...
| `scenario_session_messages` | BaseEntity | `id` (int), `scenario_session_id` (idx), `sender_id` (−1 = AI), `message_type` (`ScenarioSessionMessageType`), `content`, `start_seconds`, `end_seconds`, `metadata` (jsonb: `interrupted`, **`utteranceKind`**) | Voice transcript turns. **Not every AI-sender row is a turn of roleplay.** A thinking filler ("Hmm") and a predictive interim reply are spoken by the character and persisted here exactly like its real reply, so `metadata->>'utteranceKind'` (`'reply'`/`'filler'`/`'interim'`) is what tells them apart. Anything counting or judging AI turns must exclude `filler` and `interim` — before this marker existed the language judge scored every "Hmm" as a turn, inflating `turnsJudged` (the denominator under every error rate) and inviting annotations on utterances that are *supposed* to be contentless. Written by the live worker, so **absent means unknown, not `reply`**; older rows carry no marker and are treated as replies, which is right for a corpus that predates the filler feature. `interrupted` is written only when TRUE, on the same reasoning |
...
| `filler_judgment_sessions` | BaseEntity | `scenario_session_id`, `fillers_judged`, `distinct_phrase_ratio`, `repeated_fillers`, `dropped_annotations`, denormalised slice columns (`language`, `scenario_id`, `scenario_version_id`, `occurred_at`), `judge_model`, `judge_prompt_version`, **uniq `(session, judgeModel, judgePromptVersion)`** | **Denominator row per judged session** for the thinking-filler judge (sibling of `language_judgment_sessions`). The denominator is `fillers_judged` — **played fillers, not sessions and not turns**: a session that played forty and one that played two are not comparable units. `distinct_phrase_ratio` and `repeated_fillers` are the diversity half of the eval, counted in **plays** rather than turns because a continuation plays two fillers on one turn. A session that played fillers cleanly has a row here and no annotations, which is what distinguishes it from one never judged |
| `filler_finding_annotations` | BaseEntity | `scenario_session_id` (idx), `session_judgment_id`, `turn_index`, `dimension` (`character_fit`/`context_fit`/`safety`, idx), `category`, `severity`, `conditioned_out`, `evidence_quote`, `reasoning`, `filler_text`, `source`, `filler_type`, denormalised slice columns, `judge_model`, `judge_prompt_version` | **One row per finding against a played filler.** No scalar scores — the judge emits only labels, and rates per 100 played fillers are computed at read time (`FillerAnalyticsRepository`). Exists because filler *speed* was already measured per turn and its *quality* was not: the filler is the character's first words, so `responseLatencyMs` is measured to it, and a filler that lands instantly but sounds nothing like the character improves every latency chart while making the roleplay worse. `conditioned_out` marks a `generic_for_character` finding on a scenario that never configured a character voice — a configuration gap, not a model regression, so it is excluded from the model-facing rates and trended separately. `source` (`seed`/`predictive`/`in_turn`) is what settles whether the per-turn `in_turn` LLM call earns its cost. Re-judging under the same version DELETEs and re-INSERTs, since finding sets can shrink |
...
### 3.7 Engagement — community, badges & progress (`community`, `badge`, `progress`)
...
| `xp_events` | BaseEntity | `user_id`, `rule`, `source_type`, `source_id`, `xp` (int), `awarded_on` (date) | Append-only XP ledger for the learner Progress dashboard. Uniq `(user_id, tenant_id, rule, source_type, source_id)` is the **idempotency guarantee** — session end is reachable twice (redelivered SQS, plus the unfinalised-session sweeper) and a replay must add nothing. `source_id` is NOT NULL for that reason: Postgres treats NULLs as distinct in a unique index, so a nullable column would silently disable it. Idx `(user_id, tenant_id, awarded_on)` serves the daily caps, whose check-then-insert is additionally serialised by `XpEventRepository.lockUserDay` (`pg_advisory_xact_lock` on a hash of `(userId, tenantId)` and `awardedOn`) — otherwise two `SCENARIO_SESSION_ENDED` events for the same learner (e.g. the unfinalised-session sweeper firing them back to back) can both read the same stale daily total and both clear the cap, since the uniqueness guarantee above only dedupes one `sourceId`, not the day's running total |
| `user_progress` | BaseEntity | `user_id`, `total_xp` (int), `level` (int), `last_level_up_at`, `last_awarded_at` | Rollup of `xp_events`, written in the same transaction; the persistent nav level indicator reads it rather than re-summing the ledger. Uniq `(user_id, tenant_id)`. Holds **XP state only** — lifetime practice minutes stay in `user_daily_scores.minutes_played`, the sanctioned source the certification chart and badge ladders already read, so the three cannot disagree |

> **Tenant identity in `xp_events` / `user_progress`.** `tenant_id` is a varchar and the wider
> schema stores a tenant two ways — its uuid in some rows, its short code in others, sometimes
> both within one table (`scenario_sessions` locally holds both spellings of the same tenant).
...
| `analytics_xp_goals` | (custom — **no `tenant_id`**) | `id` (uuid), `grain` (varchar 10: `month`/`quarter`/`year`), `periodStart` (date), `targetXp` (integer), `createdAt`, `updatedAt`; uniq `(grain, periodStart)` (migration `1959000000000`) | Literal XP targets for the Analytics → Goals tab, one row per calendar period at one grain. Read-only from the application's perspective — **rows are written by migration, not through an admin form**, because a goal is a business decision to record deliberately, not application state a service should mutate. The migration that created the table ships it empty; real targets land via a follow-up migration's literal `INSERT ... ON CONFLICT ("grain", "periodStart") DO NOTHING`, matching the seeding style of `analytics_quality_thresholds`. A period with no row is not backfilled as 0 — `GoalsXpAnalyticsService` reports it as `goalXp: null, hasGoal: false` so the chart can render an explicit "no goal set" placeholder. Deliberately **not** BaseEntity, same divergence as its Highlights-tab siblings above: Goals is a platform-wide leadership view with no tenant filter |
...
| `bug_findings` | BaseWithoutTenant | `id` (uuid), `run_id` (uuid, nullable FK → `bug_hunt_runs` ON DELETE SET NULL), `repo`, `source` (`test_failure`/`lint_error`/`code_review`/`production_log`/`reported_bug`/`analytics_suggestion`/`ux_signal`), `title`/`description`/`file`/`symbol`, `evidence`, `severity`, `proven`, `touches_guarded_path`, `reported_bug_id` (uuid, nullable FK → `roadmap_opportunities` ON DELETE SET NULL), `dedupe_key`, `status`, `pr_url`, escalation `question`/`answer`/`answered_by`/`answered_at`, `decided_by`/`decided_at`, **decline reason (migration `1951000000000`):** `decision_reason` (varchar(20), nullable, CHECK `NULL` or `not_a_bug`/`duplicate`/`wrong_repo`/`wont_fix`/`too_risky`/`other`), `decision_note` (text, nullable), **admin-edited brief (migration `1916000000000`):** `original_description`, `description_edited_by` (int), `description_edited_at`, **fix-session + release:** `dispatched_at`, `session_run_url`, `session_run_id` (bigint), `release_tag`, `release_run_id` (bigint), `release_run_url`, `released_by` (int), `released_at`, **manual kill switch (migration `1911000000000`):** `cancelled_by` (int), `cancelled_at`, **coordinated fixes:** `parent_finding_id` (uuid, nullable self-FK ON DELETE CASCADE), `step_index` (int), `step_summary`, **hand-pinned stage (migration `1936000000000`):** `stage_override` (varchar, nullable, CHECK matching `roadmap_opportunities.stage`), `stage_overridden_by` (int), `stage_overridden_at`, `metadata` (jsonb) | One row per bug from any source — the comprehensive table the admin tab renders. A row can exist before any run has looked at it: `RoadmapOpportunityService.create` inserts one (`source='reported_bug'`, `status='new'`) the moment a human files a bug. `(repo, dedupe_key)` is what stops a still-open bug getting a second row across nights. An admin may rewrite `description` before starting a fix session (`PATCH /v1/bug-hunter/findings/:id/description`), because that text is the fix agent's entire brief — `buildFixSessionPrompt` states the problem to it as nothing else, and `BugHunterRepoClassifierService` reads it to pick the repo. The edit is allowed from exactly the statuses a fix session can start from, sets no status of its own, leaves the linked `roadmap_opportunities` row (the reporter's own words) untouched, and preserves the finder's text in `original_description` on the FIRST edit only. It deliberately does NOT recompute `dedupe_key`. **`dedupe_key` is built from `file` + `source` + `symbol`, never from the raw `description`** (migration `1910000000000`): the description is LLM-written, so hashing it meant the same bug worded differently on a later night hashed differently and opened a duplicate — the sweep manufactured its own reviewer noise. `symbol` (the function/class/route/component) is the stable half; where a finder supplies none, a normalised *fingerprint* of the description is used instead, which still collapses rewordings without merging two distinct bugs in one file. A finding that arrives WITH a symbol and misses also retries the symbol-less key, so rows stored before their finder learned to emit one are adopted and re-keyed rather than duplicated once at the transition. The `reported_bug_id` link also runs in reverse, best-effort: when a finding carrying it transitions to `merged` — by any of the three routes there are, the fix agent PATCHing its own `gh pr merge --admin` through `BugFindingService.setStatus` (the common one, and where the nightly sweep's auto-merges land too), `reconcilePrOpenedFindings` noticing a PR a human merged by hand, or the parent once every step of a coordinated fix has merged in `advancePlans` — the shared `releaseLinkedRoadmapOpportunity` util (`src/bug-hunter/util/`) sets the linked `roadmap_opportunities` row to `stage='released'`, `owner='Bug Hunter Agent'`, `ownerUserId=NULL`, and stamps `releasedAt` only if it wasn't already `released` — never on the `RELEASING`/`RELEASED` production-release path, which is a separate, human-gated step. **The coarse roadmap stage a reader sees on a finding is DERIVED from `status`, not stored** (`src/bug-hunter/util/bug-finding-stage.util.ts`): bugs no longer render on the roadmap board at all, so the New/Prioritised/In development/Released ladder has to be legible here, and deriving it means nothing can go stale on a screen where nobody would notice. `stage_override` is the sole exception — a bug fixed OUTSIDE Bug Hunter (a hand-written PR, a config change, a fix that rode along with other work) leaves `status` at `new` forever, and only a human can say otherwise. A pin STICKS: later transitions no longer move the stage, because the admin who set it is the only party who knows about the out-of-band fix. `PATCH /v1/bug-hunter/findings/:id/stage` with `stage: null` clears it and returns the row to derivation, wiping `stage_overridden_by`/`_at` with it **`decision_reason` is REQUIRED on every decline** — a human rejecting a bug (`POST /findings/:id/reject`) or the Verify phase refuting one (`PATCH pipeline/findings/:id` with `status='dismissed'`). It exists because a decline had two readers and served neither: the next sweep re-read the same code and re-filed the same rejected finding, so a reviewer's decision lasted one night; and nothing could state how often Bug Hunter was right. `not_a_bug`/`wrong_repo`/`duplicate` mean the FINDER was wrong and are the only ones counted against accuracy or shown back to a sweep as known non-bugs — a `wont_fix` is a priority call that may legitimately be revisited, and teaching a sweep to stop reporting real bugs would be a far more expensive failure than a duplicate filing. Nullable with NO backfill: every row declined before this migration genuinely has no reason, and `GET /v1/bug-hunter/metrics` reports those as "reason not recorded" and excludes them from its denominator rather than inventing one. **Dedup now also matches DECLINED rows** (`rejected`/`dismissed`) for 30 days, touching the existing row and bumping `metadata.rediscoveredCount` instead of inserting; `setStatus` additionally refuses to move a declined finding back into an active status, so a sweep that ignores the instruction cannot re-open a human's decision. **And it matches SHIPPED rows** (`merged`/`released`) for 30 days, which is a REGRESSION rather than a settled argument: the new row gets `metadata.regressionOf`, the old one `metadata.regressed`, both timelines get a `regressed` event and the admin gets an `action_needed` notification. Seen live before this existed — an ally-ai-learn shutdown race was fixed and released on 2026-08-28, kept firing in production, and the 2026-09-02 sweep filed it as an ordinary new bug with nothing connecting the two. A regression is only claimed on an exact dedupe-key match where the finder named a `symbol` or `file`, never on the fuzzy description fingerprint alone. **`metadata` keys with readers:** `confidence` (0-1, the Verify phase's LOWEST verifier certainty; absent on `proven` findings and on rows predating verifier scoring — below `BUG_HUNT_LOW_CONFIDENCE_THRESHOLD` the sweep holds the finding for a human even in AI mode), `verifierVotes`, `regressionOf`, `regressed`, `regressedByFindingId`, `rediscoveredCount` |
...
| Table | Base | Key columns | Notes |
|-------|------|-------------|-------|
| `roadmap_opportunities` | BaseWithoutTenant | `id` (uuid), `description` (text, CHECK ≤1000 & non-blank), `type` (`idea`/`bug`), `stage` (`new`/`prioritised`/`under_development`/`released`/`archived`), `productGoal` (text, FK **by name** → `roadmap_product_goals.name` ON UPDATE CASCADE ON DELETE RESTRICT), `owner` (text, nullable, legacy FK by name ON DELETE SET NULL), `ownerUserId` (int, nullable FK → `users` ON DELETE SET NULL), `prd`/`claudePrompt` (text, CHECK ≤20000), `releasedAt`, **month board:** `plannedMonth` (varchar(7), nullable, CHECK `^[0-9]{4}-(0[1-9]\|1[0-2])$`), `boardPosition` (int, default 0), **effort sizing (migration `1940900000000`):** `effort` (varchar(3), nullable, CHECK `NULL` or `s`/`m`/`l`/`xl`/`xxl`), **reference images (migration `1944400000000`):** `referenceImages` (jsonb, NOT NULL DEFAULT `'[]'`, CHECK: an array of ≤6 objects each carrying a non-empty string `url`), **Weaviate state:** `embeddingStatus`/`embeddingAttempts`/`embeddedAt`/`textHash`, **consumer bug reports (migration `1909000000000`):** `source` (`staff`/`consumer`, default `staff`), `tenant_id` (varchar, nullable, informational only — does NOT tenant-scope this table), `reporterContext` (jsonb, nullable — auto-captured client context: screen/appVersion/device/os/clientTimestamp), **readiness override (migration `1952000000000`):** `readinessOverriddenBy` (int, nullable), `readinessOverriddenAt` (timestamp, nullable), `readinessFailedCriteria` (jsonb, nullable), CHECK: `by` and `at` are both NULL or both set, partial index `idx_roadmap_opps_readiness_overridden` on the non-NULL rows, `createdBy`/`updatedBy` (int), `deletedAt` | The atomic unit. **Soft delete**, which is why a delete MUST also remove the vector — Postgres filters `deletedAt IS NULL`, Weaviate has no idea, and a missed delete makes duplicate detection propose a deleted row forever. `releasedAt` is stamped only on the *transition* into `released` and never re-stamped; ~173 of 280 migrated released rows have it NULL because the source trigger behaved the same way, and **nothing may backfill it**. A `bug`-type row filed by a logged-in consumer via `POST /product-roadmap/bug-reports` runs through the exact same `RoadmapOpportunityService.create()` pipeline as a staff-filed one (see `bug_findings` below) — `source='consumer'` and `createdBy` (the consumer's own `users.id`, same shared table as staff) are the only things that distinguish it. **`type='bug'` rows are never LISTED by this domain.** `EXCLUDE_BUGS_SQL` in `RoadmapOpportunityRepository` drops them from the shared `projectedQuery` (so the table, the month board and lane totals inherit it) and from the raw-SQL facets, month bounds and max-score reads; `findDuplicates` filters its candidates to `type='idea'`; and `create`/`update` skip the realtime `OPPORTUNITY_UPSERTED` broadcast for a bug so a filed one cannot flash onto a board it is not on. Bug Hunter's `bug_findings` table is where a bug is read. The row here is still written, and is still the record of who reported what with what context — `findOneWithScore` is deliberately NOT filtered, so the `?opportunity=<id>` deep link can recognise a bug id and send the reader to Bug Hunter instead of 404ing. **`effort` is a shirt size and is NULLABLE with no default and no backfill** — every row predating migration `1940900000000` is unsized, and a `DEFAULT 's'` would have asserted that 432 existing rows are the smallest thing the team does. NULL means "not estimated" and is a permanent legal state, not a gap: "unsized" and "sized as smallest" are different facts. Nothing gates on it and nothing filters or sorts by it, so it carries no index. Readable by anyone holding `VIEW_PRODUCT_ROADMAP` (it rides the shared response DTO, unstripped) and writable only through `PATCH /opportunities/:id`, which requires `EDIT_PRODUCT_ROADMAP`. **`referenceImages` is jsonb rather than a child table on purpose:** nothing links to an individual image, nothing filters or aggregates by one, and the ORDER is part of the data — so a child table would buy ids and a FK nobody reads while costing a join on the module's hottest query (the board loads every card) and a second write path inside split/merge. It is NOT NULL DEFAULT `'[]'` so "no images" has one representation and no row needed a backfill. **Every URL must resolve to the assets bucket under the `roadmap/reference-images/` prefix, enforced by `RoadmapOpportunityService.normaliseReferenceImages` on both write paths (422 otherwise)** — the array is rendered as `<img src>` in the admin dashboard for every roadmap viewer, so an unchecked URL would let one filer point every reader's browser at a host of their choosing; the objects get there only through `POST /product-roadmap/reference-images/upload-url`, which presigns a PUT at the VOTE tier (matching who may FILE, since the person with the screenshot is the person filing). `PATCH` takes the FULL resulting array, never a delta, and `[]` clears it; **removing an image never deletes the S3 object** — an orphaned PNG is a cheaper mistake than a delete that breaks a row still pointing at it. Split gives new parts an empty array, the same way it gives them a null `prd`. Not part of the embedded text, so an image change never re-bills an embedding. **The readiness checklist is enforced HERE, not only in the drawer (migration `1952000000000`).** `POST /opportunities` used to validate a description length and a product goal and save — the five criteria, the size ceiling and the staleness rule all lived in the admin bundle's `canSave`, so a `VOTE_PRODUCT_ROADMAP` token plus curl filed anything at any size, and "only managers may override" was a boolean in a React component. The gate now runs in `RoadmapOpportunityService.resolveReadiness`: `POST ai/readiness` returns its verdict SIGNED (`RoadmapReadinessTokenService` — HMAC-SHA256 under a key derived from `JWT_ACCESS_SECRET`, domain-separated, binding the trimmed description, the product goal, a fingerprint of the criteria set, and a 30-minute expiry), and the create call hands that token back as `readinessToken`. **Signed rather than re-graded on create on purpose:** re-grading puts an LLM call in a write path, makes a model outage refuse filings, and lets a second reading disagree with the one the filer was shown. A tampered, expired, stale or criteria-mismatched token is always a 400; an ABSENT one is allowed with a `filed with no readiness token` warning until `ROADMAP_READINESS_REQUIRE_TOKEN` flips true, which is a one-release rollout window for the client that predates the field. **The size half of the gate is applied to the effort being FILED, not the one in the token** — correcting a size the model got wrong is a documented exemption (a re-run would recompute the size and overwrite the correction), so filing an S the model called XL is allowed and filing an XL is not. A failing verdict needs `readinessOverride` AND the FULL manage tier — `RoadmapAccessService.canManageBoard`, the permission AND the `product_roadmap_manage` toggle, because since the role collapse `EDIT_PRODUCT_ROADMAP` sits on every platform admin and the toggle is the entire distinction; a non-manager asking for one is a 403. That is what the three `readiness*` columns record, and they are **write-only for now** — queryable in SQL through the partial index ("what got waved through?") but not yet on the response DTO. `/bug-reports` shares `create()` and opts OUT (`enforceReadiness` is passed only by `POST /opportunities`): a one-prompt consumer form has never shown a checklist, and grading one against "names the user group it affects" would refuse every real report |
...
| `roadmap_strategy_goals` | BaseWithoutTenant | `id` (uuid), `name` (text UNIQUE, CHECK non-blank & ≤200), `position` (int, default 0) | **A different concept from `roadmap_product_goals`, and the names are close enough to be worth stating.** That table is a CATEGORY — exactly one per opportunity, used to file and filter ("Scribe"). This one is an OUTCOME the board is RANKED against ("Cut time-to-first-value"), and one opportunity may advance several or none. That many-to-many is the whole point and is why it could not be folded into the category table — doing so would have forced the category many-to-many and broken every saved view whose `goalFilter` stores category names. FK target for `roadmap_opportunity_goal_impacts.goalName` (ON UPDATE CASCADE), so a rename carries every stored verdict across for free. No soft delete, same reason as the category table: a soft-deleted row still satisfies the FK. Capped at 12 (`ROADMAP_RANK.MAX_STRATEGY_GOALS`) — coverage divides by this count, so it means less the more goals there are. Migration `1944100000000` |
| `roadmap_opportunity_goal_impacts` | BaseWithoutTenant | `id` (uuid), `opportunityId` (uuid FK → `roadmap_opportunities` ON DELETE CASCADE), `goalName` (text, FK **by name** → `roadmap_strategy_goals.name` ON UPDATE CASCADE **ON DELETE CASCADE**), `helped` (bool), `reason` (text, nullable, CHECK ≤500), `assessedAt`. UNIQUE `(opportunityId, goalName)` | One LLM verdict per (opportunity, strategy goal) — the composite rank's fourth factor. **Coverage is `COUNT(*) FILTER (WHERE helped) / (live goal count)`, computed per read and NEVER stored:** a stored percentage is only meaningful against the goal list it came from, and that list changes. Verdict rows make two of the three mutations free — a goal DELETED cascades its verdicts away and coverage recomputes, a goal RENAMED cascades across, and only a goal ADDED needs the model. That last case is why `assessedAt` and the `assessed < live goal count` staleness check exist: an under-assessed row is *detectable* rather than silently dividing by the wrong number. **Not user-editable by design** — a ranking input anyone could hand-edit is one people would edit to move their own idea up; the correction path is re-running the assessment. This is the one place the module differs from `effort`, which is explicitly a proposal the filer may correct. Written by `RoadmapGoalImpactService` on file, on description edit, and on explicit reassess — never automatically when the strategy changes, because that would re-bill the whole board as a side effect of typing a goal name. Migration `1944100000000` |
| `roadmap_rank_weights` | BaseWithoutTenant | `id` (int, **CHECK `= 1`** — singleton), `votesWeight`/`votersWeight`/`effortWeight`/`goalImpactWeight` (int, CHECK 0–10 each, CHECK sum > 0). Defaults 3/3/1/3 | The four composite-rank factor weights, normalised to sum 1 at read time so an admin can type 3/3/1/3 without doing arithmetic. `id` is CHECK-pinned rather than a uuid so "there is exactly one row" is a database guarantee — a second row would make the ranking depend on read order. The sum > 0 CHECK stops a divide-by-zero in the ranking SQL. **A table rather than the per-tenant `Preference` recipe**: the roadmap is global with no tenant, and these are read by the list query on every board load. **Effort defaults LOW (1) on purpose** — it is an INVERSE factor (cheaper ranks higher), so weighting it heavily surfaces trivial work above hard high-conviction work; at 1 it breaks ties rather than driving the order. Changing a weight costs nothing but a re-sort — it never re-runs the LLM. Migration `1944100000000` |
...
| Table | Base | Key columns | Notes |
|-------|------|-------------|-------|
| `builder_sessions` | BaseWithoutTenant | `id` (uuid), `title` (varchar(200)), `slug` (varchar(80) UNIQUE), `status` (varchar CHECK: `INTERVIEWING`/`PRD_READY`/`BUILDING`/`WAITING_FOR_INPUT`/`COMPLETED`/`FAILED`/`CANCELLED`), `currentStage` (varchar CHECK: `SETUP`…`DONE`, nullable), `repos` (jsonb), `engine`/`model`, `lastMessageSeq`, `lastRunSequence` (int), `budgetUsd`/`totalCostUsd` (numeric), `runnerMinutes` (int), `tenant_id` (varchar, nullable), `archivedAt` (timestamp, nullable), `createdBy`/`updatedBy`, `deletedAt` | One interview → zero or more build runs. `slug` is the base of every branch the session pushes (`builder/<slug>`) and its unique index **includes soft-deleted rows** — the remote branch outlives the row, so reusing a slug would push onto someone else's history. `lastMessageSeq` and `lastRunSequence` are both atomic `UPDATE … RETURNING` counters, behind message-seq and run-number allocation respectively — run numbers used to be read-then-incremented off `MAX(sequence)`, so a double-clicked answer could dispatch two runners at the same branches. `archivedAt` is a per-creator feed filter set only from COMPLETED/FAILED/CANCELLED — archiving hides a session from its owner's default list view and changes nothing else: not `status`, not the PRD/transcript/runs/events/PRs/reports underneath it, and not the tenant caps in `assertTenantWithinCaps` (which count by status, not archive state) |
...
- **AWS S3**: binary assets. Object keys are stored on Postgres rows — `storage_key`
  ...
  images, logos), plus one jsonb array of them (`roadmap_opportunities.referenceImages`, under the
  `roadmap/reference-images/` prefix of the assets bucket). To find a file, read the key/URL off
  the owning row. Note the reverse does not hold: nothing sweeps the buckets, so an object whose
  row stopped pointing at it simply stays there.
...
| How the roadmap RANKS an opportunity | `roadmap_rank_weights` (the four weights) + `roadmap_opportunity_goal_impacts` (goal coverage) + `roadmap_allocations` (votes AND distinct backers). All four factors are computed in SQL per read — **there is no stored composite score** |
...
- Quick entity census: `find src -name '*.entity.ts' | wc -l`
  (**209** at this reconcile, including the two thinking-filler judgment entities added here.
  The previous note said 201 and had drifted by six — run the command rather than
  trusting the number, and correct it when you do).
