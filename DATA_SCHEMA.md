...
| `scenario_session_turn_metrics` | BaseEntity | `scenario_session_id`/`room_id`, `turn_index`, **`response_latency_ms`**, `eou_delay_ms`, `llm_ttft_ms`, `tts_ttfb_ms`, `orchestration_ms`, `llm_response_ms`, `prosody_ms`, `branching_ms`, `knowledge_retrieval_ms`, `process_events_ms`, `behaviors_ms`, `llm_model`, `language`, `env`, `events_detected`, `interrupted`, `llm_timed_out`, `occurred_at` | **Per-turn latency telemetry** — wide table built for Metabase percentile dashboards. Indexed on `scenario_session_id`, `occurred_at`, `scenario_id` |
