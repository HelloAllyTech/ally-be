...
| `llm_task_configs` | BaseWithoutTenant (**no `tenant_id`**) | `id` (uuid), `task_id` (varchar 100, **uniq** `idx_llm_task_configs_task_id`), `provider` (varchar 50, nullable), `model` (varchar 100, nullable), `temperature` (double precision, nullable), `fallback_enabled` (bool, default true), `updated_by` (uuid, nullable), `note` (text, nullable), `createdAt`, `updatedAt` (migration `1956000000000`) | Per-AI-task model selection, editable by an admin without a deploy. Keyed by **AI-task-registry row id** (`drift-judge`, `interim-reply`, ...) and deliberately **not** by `LlmTask`: that label is for usage analytics and does not identify a call — 16 registry rows carry no label at all, and `agent_turn` covers four rows including `interim-reply`/`predictive-filler`, which run a cheap model on purpose. Keying on the label would collapse those into one setting and silently retier the live voice path. Every selection column is nullable, meaning "ask the next layer down", the same convention `prompts.model` and `lab_skills.model` use; the full chain is *explicit call argument → prompt row → this row → platform tier (`LLM_FAST_MODEL`/`LLM_REASONING_MODEL`) → compiled-in OpenAI floor*, resolved by `LlmTargetResolverService` which also reports **which** rung won. `fallback_enabled=false` opts a task out of the retry-on-OpenAI path — required for anything whose output is stored and trended (the judges), where a quiet substitution corrupts a series only comparable within one `(MODEL, PROMPT_VERSION)` pair. **No rows are seeded**: an absent row is the pre-existing behaviour, so the table changes nothing until someone edits it. Platform-wide, no `tenant_id` — which model serves a task is a platform decision, and a per-tenant model would make cost and quality per task unreadable |
