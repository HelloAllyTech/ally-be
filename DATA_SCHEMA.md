...
| `copilot_sessions` | BaseWithoutTenant | `specId` (idx), `status` (ACTIVE/ENDED), `mode` (BUILDING/ITERATING — authoring interview vs live-test feedback refinement), `lastMessageSeq` (atomic per-session message counter), `metadata`, `created_by`/`updated_by`, `deleted_at` | One copilot conversation over a spec |
