...
`roleplay_specs`/`roleplay_spec_versions`, because both the agent (`update_prd` patches) and the admin
(direct section edits) mutate one document — the snapshots are the only way to answer "who changed this
requirement?" afterwards.

| Table | Base | Key columns | Notes |
|-------|------|-------------|-------|
| `builder_sessions` | BaseWithoutTenant | `id` (uuid), `title` (varchar(200)), `slug` (varchar(80) UNIQUE), `status` (varchar CHECK: `INTERVIEWING`/`PRD_READY`/`BUILDING`/`WAITING_FOR_INPUT`/`COMPLETED`/`FAILED`/`CANCELLED`), `currentStage` (varchar CHECK: `SETUP`…`DONE`, nullable), `repos` (jsonb), `engine`/`model`, `lastMessageSeq` (int), `budgetUsd`/`totalCostUsd` (numeric), `runnerMinutes` (int), `tenant_id` (varchar, nullable), `createdBy`/`updatedBy`, `deletedAt` | One interview → zero or more build runs. `slug` is the base of every branch the session pushes (`builder/<slug>`) and its unique index **includes soft-deleted rows** — the remote branch outlives the row, so reusing a slug would push onto someone else's history. `lastMessageSeq` is the atomic counter behind message seq allocation |
| `builder_messages` | BaseWithoutTenant | `id` (uuid), `sessionId` (uuid FK ON DELETE CASCADE), `seq` (int), `role` (`user`/`assistant`), `content` (text), `toolCalls`/`toolResults`/`metadata` (jsonb). UNIQUE `(sessionId, seq)` | Append-only, **no soft delete** — history must replay faithfully into the Anthropic messages array (text + tool_use + tool_result blocks rebuilt from the jsonb columns). `seq` is allocated by an atomic `UPDATE builder_sessions SET lastMessageSeq = lastMessageSeq + 1 RETURNING` inside the insert transaction, so concurrent appenders cannot collide. Question payloads live in the assistant row's `metadata.questions`; the raw answer payload lives on the user row |
...
| `builder_lessons` | BaseWithoutTenant | `id` (uuid), `sessionId` (uuid, **no FK**), `repo` (varchar, nullable = platform-wide), `category` (CHECK `gotcha`/`convention`/`estimate`/`process`), `lesson` (text) | Cross-session memory from build retrospectives, fed back into later prompts. No FK to `builder_sessions` on purpose: a lesson outliving the session that learned it is the entire point |
