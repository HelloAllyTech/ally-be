...
export function buildFixSessionPrompt({
  ...
  return [
    ...
    `5. Run the full suite: ${verifyCommandsList(commands)}. All of them must be green${commands.typecheck ? ' — a fix that only passes the test suite and lint but leaves a type error will pass this step and then fail the moment the PR opened in step 8 tries to merge, which is worse than catching it now' : ''}. If not, retry the fix (up to ${BUG_HUNT_MAX_FIX_ATTEMPTS} attempts in total). If it is still not green after that, run ${report('escalated', 'suite still red after the attempt cap')}, then ${patch({ status: 'failed' })}, and finish with outcome "failed". Never force a merge past a red suite and never keep retrying past the cap.`,
