...
/**
 * Promotes a single named user from SUPER_ADMIN to SUPER_DUPER_ADMIN.
 *
 * The SUPER_DUPER_ADMIN group and its permission set are created by
 * 1828000000000-AddSuperDuperAdminRole (which also cloned SUPER_ADMIN's
 * permissions onto it), so this migration only reassigns the user: it adds the
 * SUPER_DUPER_ADMIN membership, then drops SUPER_ADMIN.
 *
 * The reassignment is keyed on the user's email and is a no-op in any
 * environment where that account (or the group) does not exist — e.g. fresh
 * local databases — so it is safe to run everywhere. It is idempotent.
 *
...
const PROMOTE_EMAIL = 'gopikrishnan.sasikumar@helloally.ai';

export class PromoteGopikrishnanToSuperDuperAdmin1838000000000 implements MigrationInterface {
