...
/**
 * One user's coin allocation to one opportunity in one calendar month. This table IS the
 * priority signal — everything else on the board is metadata around it.
 *
 * Invariants, and where each is enforced:
 *
 *  - SUM(coins) per (userId, periodKey) ≤ 100 — a cross-row invariant, so it lives in the
 *    trigger roadmap_enforce_monthly_cap() (migration 1871000000001) PLUS a
 *    pg_advisory_xact_lock in RoadmapAllocationService. Neither layer alone is sufficient:
 *    a CHECK cannot see other rows, and a service-only read-then-write races under READ
 *    COMMITTED.
 *  - 0 ≤ coins ≤ 100 — CHECK constraint. Note the trigger does NOT catch a negative value
 *    (0 + -5 is under the cap), so this CHECK is the only guard there.
 *  - periodKey matches ^[0-9]{4}-(0[1-9]|1[0-2])$ — CHECK constraint.
 *  - coins may only be allocated to a stage=new opportunity — service-level, because
 *    split/merge must redistribute coins on opportunities that have already moved on. In the
 *    source this was a trigger that split/merge defeated with a transaction-local GUC
 *    (app.bypass_stage_check); that hack is gone.
 *
 * NO soft delete: setting coins to 0 deletes the row. A soft-deleted allocation would still
...
// The priority-score aggregate: SUM(coins) GROUP BY "opportunityId".
